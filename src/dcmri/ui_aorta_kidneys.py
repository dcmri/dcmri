...
from dcmri import sig, utils, ui, kidney, ConcAorta
...
class AortaKidneys(ui.SuperModel):
    ...
    def _predict_kidneys(self, time):
        ...
        return (
            utils.sample(time[0], self._t, self._Sk['lk'], p['TS']),
            utils.sample(time[1], self._t, self._Sk['rk'], p['TS']),
        )
