...
def _par_values(self, *args, tiss=False, kin=False, seq=False,
    ...
    if tiss:
        return {p: pars[p] for p in
                tissue.params_relax_tissue(self.kinetics, self.water_exchange)}
    if kin:
        return {p: pars[p] for p in 
                tissue.params_relax_tissue(self.kinetics, 'FF')}
    if seq:
        return {p: pars[p] for p in _seq_pars(self.sequence)}
    if export:
        p0 = _model_pars(self.kinetics, self.water_exchange, self.sequence)
        p1 = tissue.params_relax_tissue(self.kinetics, self.water_exchange)
...
def _model_pars(kin, wex, seq):
    pars = ['r1']
    pars += ['S0a','R10a', 'B1corr_a','T_a']
    pars += _seq_pars(seq)
    pars += ['TS']
    pars += tissue.params_relax_tissue(kin, wex)
    pars += ['R10', 'n0']
    return pars
