...
def test_coverage_readout():
    ...
    for cnfgs in itertools.product(*values):
        ...
        read(Mz=[1], R2=[1], R2s=[1])
        # 2c array
        read(Mz=[1,1], R2=[1,1], R2s=[1,1])
        # 2c + time
        read(Mz=np.ones((2,3)), R2=np.ones((2,3)), R2s=np.ones((2,3)))


def test_exceptions_readout():

    try:
        Readout('GE-EPI')(Mz=None)
    except:
        pass
    else:
        assert False
    ...
    try:
        Readout('DE-EPI')(Mz=np.ones(3), R2=np.ones(3), R2s=np.ones(2))
    except:
        pass
    else:
        assert False
...
def test_exceptions_longitudinal():
    try:
        Longitudinal('3D-SPGR-SS')(R1=None)
    except:
        pass
    else:
        assert False
    ...
    try:
        Longitudinal('3D-SPGR-SS')(R1=np.ones(3), v=[0.5, 0.5])
    except:
        pass
    else:
        assert False
