...
def test_mz_readout():
    ...
    result = seqs.mz_readout(Mz, R2, S0, FA, TE, noise_sdev)
...
def test_Mz_spgr_in_ss():
    ...
    # Case 1: Infinite TR shortcut
    res_inf = seqs.Mz_spgr_in_ss(R1, v, Fw, j, me, np.inf, FA)
    np.testing.assert_array_almost_equal(res_inf, np.full_like(R1, me))
    
    # Case 2: Running actual underlying pulse simulation loop
    res_finite = seqs.Mz_spgr_in_ss(R1, v, Fw, j, me, TR=0.1, FA=FA)
    assert res_finite.shape == (2, 3)
...
def test_Mz_pr_spgr():
    ...
    res = seqs.Mz_pr_spgr(R1, v, Fw, j, me, TC, TR, FA, TP, TA, PA)
...
def test_Mz_pr_spgr_in_ss():
    ...
    res = seqs.Mz_pr_spgr_in_ss(R1, v, Fw, j, me, TC, TR, FA, TP, TA, PA)
    assert res.shape == (2, 3)
...
def test_Mz_ssi():
    ...
    j_1d = np.ones((1, 3))
    v = 1
    res_1d = seqs.Mz_ssi(R1_1d, v, Fw, j_1d, me, TR, FA, TF, SA)
    ...
    v = np.ones(2) / 2
    Fw = np.identity(2)
    res_2d = seqs.Mz_ssi(R1_2d, v, Fw, j_2d, me, TR, FA, TF, SA)
