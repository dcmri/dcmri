...
def test_fake_aif():
    ...
    time, aif, _ = dc.fake_aif(model='3D-SR-SPGR-SS')
...
def test_fake_brain():
    ...
    time, signal, aif, gt = dc.fake_brain(n=8, model='3D-SR-SPGR-SS', verbose=1)
    assert 2680 < trapezoid(aif, time) < 2690
...
def test_fake_tissue():
    ...
    time, aif, roi, gt = dc.fake_tissue(model='3D-SR-SPGR-SS')
...
def test_fake_liver():
    ...
    time, aif, vif, roi, gt = dc.fake_liver(sequence='3D-SPGR-SSI')
    assert 1500 < trapezoid(aif, time) < 1600
    assert 1500 < trapezoid(vif, time) < 1600
    assert 2000 < trapezoid(roi, time) < 2020
...
def test_fake_tissue2scan():
    ...
    time, aif, roi, gt = dc.fake_tissue2scan(model='3D-SR-SPGR-SS')
...
def test_fake_kidney():
    ...
    time, aif, roi, gt = dc.fake_kidney(model='3D-SPGR-SS')
