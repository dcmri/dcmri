...
def test_Mz_dyn_spgr():
    ...
    t, Mz = functions_dynamic.Mz_dyn_spgr(tR1, R1, v, Fw, j, me, TR, FA, Nph, tj=tR1)
...
def test_Mz_dyn_pr_spgr():
    ...
    tR1 = dt * np.arange(R1.shape[1])
    t, Mz = functions_dynamic.Mz_dyn_pr_spgr(tR1, R1, v, Fw, j, me, TR, FA, Nph, TP, TD, PA, tj=tR1)
    assert Mz.shape == (2, 21, 129)
    assert t.shape == (21, 129)

def test_Mz_dyn_pr_spgr_ss():
    ...
    tR1 = dt * np.arange(R1.shape[1])
    t, Mz = functions_dynamic.Mz_dyn_pr_spgr_ss(tR1, R1, v, Fw, j, me, TR, FA, Nph, TP, TD, PA, tj=tR1)
    assert Mz.shape == (2, 21, 129)
    assert t.shape == (21, 129)

def test_Mz_dyn_spgr_ss():
    """Test Mz_dyn_spgr_ss shortcut branch and standard loop shape."""
    dt = 10.0
    R1 = np.ones((2, 3))
    j = np.ones((2, 3))
    v = np.ones(2) / 2
    ...
    tR1 = dt * np.arange(R1.shape[1])
    t, Mz = functions_dynamic.Mz_dyn_spgr_ss(tR1, R1, v, Fw, j, me, TR, FA, Nph, tj=tR1)
...
def test_Mz_dyn_se():
    ...
    # Running actual underlying pulse simulation loop
    tR1 = dt * np.arange(R1.shape[1])
    t, Mz = functions_dynamic.Mz_dyn_se(tR1, R1, v, Fw, j, me, TE, TR, FA, tj=tR1)
    assert Mz.shape == (2, 10, 2)
    assert t.shape == (10, 2)

def test_Mz_dyn_spgr_ssi():
    ...
    t, Mz_1d = functions_dynamic.Mz_dyn_spgr_ssi(tR1, R1_1d, v, Fw, j_1d, me, TR, FA, Nph, TF, SA, tj=tR1)
    ...
    t, Mz_2d = functions_dynamic.Mz_dyn_spgr_ssi(tR1, R1_2d, v, Fw, j_2d, me, TR, FA, Nph, TF, SA, tj=tR1)
