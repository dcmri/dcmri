...
from dcmri import SignalToConc  
import dcmri as dc
...
def test_coverage():
    ...
    # 1D signals (n_times)
    for seq in list(dc.SEQUENCES.keys()) + ['lin']:
        if seq != 'lin':
            if not dc.SEQUENCES[seq]['steady-state']: # Not invertible
                continue 
        print('scalar: ',seq)
...
def test_function():
    ...
    seqs_dce = [s for s, v in dc.SEQUENCES.items() if v['type']=='DCE' and s!='SSI' and v['steady-state']]
    seqs_ssi = ['3D-SPGR-SSI']
    seqs_dsc = [s for s, v in dc.SEQUENCES.items() if v['type']=='DSC' and v['steady-state']]

    # Define signal parameters
    R10a, S0a, B1a = 0.7, 3, 0.75
    R20s, R20 = 1.5, 2.0
    rp = dc.relaxivity(3, 'blood', 'gadoterate')
    r2 = 10000
    r2s = 15000
    ...
    for sequence in seqs_ssi:
        # Generate signal and reconstruct concentrations
        R1a = R10a + rp * ca
        S = dc.Signal(sequence, **params_ssi)(
            R1=R1a, S0=S0a, B1corr=B1a, TE=0,
        )
        ca_rec = dc.SignalToConc(sequence, **params_ssi)(
            S, R10=R10a, r1=rp, B1corr=B1a, TE=0, 
        )
