...
def test_coverage():
    ...
    for seq in dc.SignalToConc.configs['sequence']:
        ...
        sig2conc = dc.SignalToConc(seq, defaults=DEFAULTS)
        sig2conc.params()
    ...
    for seq in dc.SignalToConc.configs['sequence']:
        ...
        # Tst Options
        if seq == 'lin':
            dc.SignalToConc(seq, False, defaults=DEFAULTS)(signal, S0=np.full(n_samples, 1))
            dc.SignalToConc(seq, True, defaults=DEFAULTS)(signal)

        if seq in ['ZTE-3D-IR-SPGR-SS', '3D-IR-SPGR-SS']:
            dc.SignalToConc(seq, False, defaults=DEFAULTS)(signal, S0=np.full(n_samples, 1))

        if seq in ['Eq-GE-EPI', 'Eq-SE-EPI', 'Eq-DE-EPI', 'GE-EPI', 'SE-EPI', 'DE-EPI']:
            dc.SignalToConc(seq, True, defaults=DEFAULTS)(signal, R1b=np.full(n_samples, 1))
            dc.SignalToConc(seq, False, defaults=DEFAULTS)(signal, S0=np.full(n_samples, 1))
...
def test_function():
    ...
    for sequence in seqs_dce:
        ...
        R1a = R1ba + rp * ca
        R2s = R2sb + r2s * ca
        S = dc.Signal(sequence, defaults=params_dce)(
            R1=R1a, R2s=R2s, S0=S0a, B1corr=B1a,
        )
        ca_rec = dc.SignalToConc(sequence, defaults=params_dce)(
            S, R1b=R1ba, r1=rp, B1corr=B1a, 
        )
    ...
    for sequence in seqs_dsc:
        ...
        R2s = R2sb + r2s * ca
        R2 = R2b + r2 * ca
        S = dc.Signal(sequence, defaults=params_dsc)(
            R1=R1a, R2=R2, R2s=R2s, S0=S0a, B1corr=B1a, TR=np.inf,
        )

        ca_rec = dc.SignalToConc(sequence, defaults=params_dsc)(
    ...
    ca_rec = dc.SignalToConc('lin', defaults=DEFAULTS)(
        S, R1b=R1ba, r1=rp, 
    )
...
def test_brain():
    ...
    R1b = R1ba + rp * cb
    aif_ = dc.Signal(seq, defaults=DEFAULTS)(S0=S0, R1=R1b, TR=TR, FA=FA)
    time = np.arange(0, tacq, dt)
    aif_acq = dc.sample(time, tsim, aif_, dt)

    # Invert
    cb_rec = dc.SignalToConc(seq, defaults=DEFAULTS)(aif_acq, R1b=R1ba, n0=1, TR=TR, FA=FA, r1=rp)
