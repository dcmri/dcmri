...
def test_coverage():
    ...
    for seq in SignalToConc.configs['sequence']:
        print('scalar: ',seq)
...
def test_brain():
    ...
    R1b = R10a + rp * cb
    aif_ = Signal(seq)(S0=S0, R1=R1b, TR=TR, FA=FA)
    time = np.arange(0, tacq, dt)
    aif_acq = sample(time, tsim, aif_, dt)

    # Invert
    cb_rec = SignalToConc(seq)(aif_acq, R10=R10a, n0=1, TR=TR, FA=FA, r1=rp)
