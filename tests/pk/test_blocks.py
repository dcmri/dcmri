...
def test_prop_trap():
    t = np.linspace(0, 100, 20)
    h = blocks.prop_trap(t)
    assert np.linalg.norm(h) == 0

def test_conc_trap():
    t = np.linspace(0, 100, 20)
    J = np.ones(len(t))
    C = blocks.conc(J, t=t, model='trap')
    assert C[-1] == 100
    C = blocks.conc(J, dt=t[1], model='trap')
    assert C[-1] == 100
    C0 = convolution.conv(blocks.res_trap(t), J, t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) == 0

def test_flux_trap():
    t = np.linspace(0, 100, 20)
    J = np.ones(len(t))
    Jo = blocks.flux(J, model='trap')
    assert np.linalg.norm(Jo) == 0
    Jo0 = convolution.conv(blocks.prop_trap(t), J, t)
    assert np.linalg.norm(Jo-Jo0) == 0

def test_res_pass():
    T = 30
    t = np.linspace(0, 100, 20)
    r = blocks.res_pass(T, t)
...
def test_conc_pass():
    ...
    t = np.linspace(0, 100, 20)
    J = np.ones(len(t))
    C = blocks.conc(J, T, model='pass')
    assert np.unique(C)[0] == T
    C0 = convolution.conv(blocks.res_pass(T,t), J, t)
...
def test_flux_pass():
    ...
    t = np.linspace(0, 100, 20)
    J = np.ones(len(t))
    Jo = blocks.flux(J, model='pass')
    assert np.linalg.norm(J-Jo)/np.linalg.norm(J) < 1e-12
    Jo0 = convolution.conv(blocks.prop_pass(t), J, t)
...
def test_conc_comp():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_comp(T,t), J, t)
    C = blocks.conc(J, T, t=t, model='comp')
    assert np.linalg.norm(C[1:]-C0[1:])/np.linalg.norm(C0[1:]) < 1e-2
    C = blocks.conc(J, T, dt=t[1], model='comp')
    assert np.linalg.norm(C[1:]-C0[1:])/np.linalg.norm(C0[1:]) < 1e-2
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_comp(T,t), J, t)
    ...
    C = blocks.conc(J, np.inf, t=t, model='comp')
    C0 = blocks.conc(J,t=t, model='trap')
    assert np.linalg.norm(C[1:]-C0[1:])/np.linalg.norm(C0[1:]) < 1e-1
...
def test_flux_comp():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_comp(T,t), J, t)
    Jo = blocks.flux(J, T, t=t, model='comp')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-2
    Jo = blocks.flux(J, T, dt=t[1], model='comp')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-2
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_comp(T,t), J, t)
    ...
    Jo = blocks.flux(J, np.inf, t=t, model='comp')
    assert np.linalg.norm(Jo) == 0

def test_res_plug():
    T = 25
    t = np.linspace(0, 150, 20)
    r = blocks.res_plug(T, t)
...
def test_conc_plug():
    ...
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_plug(T,t), J, t)
    C = blocks.conc(J, T, t=t, model='plug')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2
    C = blocks.conc_plug(J, T, t, solver='conv')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2
    C = blocks.conc_plug(J, T, dt=t[1])
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_plug(T,t), J, t)
    C = blocks.conc_plug(J, T, t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1
    C = blocks.conc_plug(J, np.inf, t)
    C0 = blocks.conc_trap(J)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2
    C = blocks.conc_plug(J, 0, t)
...
def test_flux_plug():
    ...
    t = np.linspace(0, 150, 40)
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_plug(T,t), J, t)
    Jo = blocks.flux(J, T, t=t, model='plug')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 0.2
    Jo = blocks.flux(J, T, t=t, model='plug', solver='conv')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 0.2
    Jo = blocks.flux(J, T, dt=t[1], model='plug')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 0.2
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_plug(T,t), J, t)
    ...
    Jo = blocks.flux(J, np.inf, t=t, model='plug')
    assert np.linalg.norm(Jo) == 0
    Jo = blocks.flux(J, 0, t=t, model='plug')
    assert np.linalg.norm(Jo-J) == 0

    try:
        Jo = blocks.flux(J, T, t=t, model='plug', solver='blabla')
    except:
        assert True
    else:
        assert False
...
def test_prop_chain():
    ...
    try:
        h = blocks.prop_chain(-1, D, t)
    except:
        assert True
    else:
        assert False
    try:
        h = blocks.prop_chain(T, -1, t)
    except:
        assert True
    else:
        assert False
    try:
        h = blocks.prop_chain(T, 2, t)
    except:
        assert True
    else:
        assert False
    h = blocks.prop_chain(T, 0, t)
    h0 = blocks.prop_plug(T,t)
    assert np.linalg.norm(h-h0)==0
    h = blocks.prop_chain(T, 1, t)
    h0 = blocks.prop_comp(T, t)
    assert np.linalg.norm(h-h0)==0
...
def test_res_chain():
    ...
    D = 0.5
    t = np.linspace(0, 150, 500)
    h = blocks.res_chain(T, D, t)
    assert (trapezoid(h,t)-T)**2/T**2 < 1e-6
    t = [0,5,15,30,60,90,150]
    h = blocks.res_chain(T, D, t)
    assert (trapezoid(h,t)-T)**2/T**2 < 1e-3
    h = blocks.res_chain(T, 0, t)
    h0 = blocks.res_plug(T,t)
    assert np.linalg.norm(h-h0)==0
    h = blocks.res_chain(T, 1, t)
    h0 = blocks.res_comp(T, t)
    assert np.linalg.norm(h-h0)==0
...
def test_conc_chain():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_chain(T,D,t), J, t)
    C = blocks.conc(J, T, D, t=t, model='chain')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    C = blocks.conc_chain(J, T, D, dt=t[1])
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_chain(T,D,t), J, t)
    C = blocks.conc_chain(J, T, D, t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    h = blocks.conc_chain(J, T, 0, t)
    h0 = blocks.conc_plug(J, T,t)
    assert np.linalg.norm(h-h0)==0
    h = blocks.conc_chain(J, T, 1, t)
    h0 = blocks.conc_comp(J, T, t)
    assert np.linalg.norm(h-h0)==0
...
def test_flux_chain():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_chain(T,D,t), J, t)
    Jo = blocks.flux(J, T, D, t=t, model='chain')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    Jo = blocks.flux(J, T, D, dt=t[1], model='chain')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_chain(T,D,t), J, t)
    Jo = blocks.flux(J, T, D, t=t, model='chain')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    h = blocks.flux(J, T, 0, t=t, model='chain')
    h0 = blocks.flux(J, T, t=t, model='plug')
    assert np.linalg.norm(h-h0)==0
    h = blocks.flux(J, T, 1, t=t, model='chain')
    h0 = blocks.flux(J, T, t=t, model='comp')
    assert np.linalg.norm(h-h0)==0
...
def test_flux_pfcomp():
    ...
    Jo = blocks.flux(J, T, D, t=t, model='pfcomp')
    assert np.sum(Jo)*t[1]-1 < 1e-2

    # D=0
    Jo = blocks.flux(J, T, 0, t=t, model='pfcomp')
    assert np.sum(Jo)*t[1]-1 < 1e-2

    # D=1
    Jo = blocks.flux(J, T, 1, t=t, model='pfcomp')
    assert np.sum(Jo)*t[1]-1 < 1e-2
    ...
    try:
        ...
        assert False
    try:
        Jo = blocks.flux(J, T, 2, t=t, model='pfcomp')
    except:
        assert True
    else:
        assert False
...
def test_prop_step():
    ...
    h = blocks.prop_step(T, D, t)
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    assert np.abs(trapezoid(t*h,t)-T) < 1e-12
    h = blocks.prop_step(T, 1, t)
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    h = blocks.prop_step(120, 0.5, t)
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    h = blocks.prop_step(120, 0.5, t)
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    t = [0,5,15,30,60,90,150]
    ...
    try:
        ...
        assert False
    try:
        h = blocks.prop_step(T, 2, t)
    except:
        assert True
    else:
        assert False
    h = blocks.prop_step(np.inf, D, t)
    assert np.linalg.norm(h)==0
    h = blocks.prop_step(T, 0, t)
    h0 = blocks.prop_plug(T, t)
    assert np.linalg.norm(h-h0)==0
...
def test_conc_step():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_step(T,D,t), J, t)
    C = blocks.conc(J, T, D, t=t, model='step')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    C = blocks.conc_step(J, T, D, dt=t[1])
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_step(T,D,t), J, t)
    C = blocks.conc_step(J, T, D, t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    C = blocks.conc_step(J, T, 0, t)
    C0 = blocks.conc_plug(J, T, t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
...
def test_flux_step():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_step(T,D,t), J, t)
    Jo = blocks.flux(J, T, D, t=t, model='step')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    Jo = blocks.flux(J, T, D, dt=t[1], model='step')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_step(T,D,t), J, t)
    Jo = blocks.flux(J, T, D, t=t, model='step')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    Jo = blocks.flux(J, T, 0, t=t, model='step')
    J0 = blocks.flux(J, T, t=t, model='plug')
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12


def test_prop_free():
    t = np.linspace(0, 150, 100)
    h = blocks.prop_free([1,1], t, TT=[30,60,90])
    T = 60
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    assert (trapezoid(t*h,t)-T)**2/T**2 < 1e-9
    h = blocks.prop_free([2,2], t, TT=[30,60,90])
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    assert (trapezoid(t*h,t)-T)**2/T**2 < 1e-9
    h = blocks.prop_free([2,5], t, TT=[30,60,90])
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    h = blocks.prop_free([2,5], t, TT=[30,60,120])
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    h = blocks.prop_free([2,5], t, TT=[30.5,60.5,120.5])
    assert np.abs(trapezoid(h,t)-1) < 1e-12
    h = blocks.prop_free([1,1], t)
    h0 = blocks.prop_free([1,1], t, TTmax=np.amax(t))
    assert np.linalg.norm(h-h0)/np.linalg.norm(h0) < 1e-12
    try:
        h = blocks.prop_free([1,1,1], t, TT=[30,60,90])
    except:
        assert True
    else:
        assert False
...
def test_res_free():
    t = np.linspace(0, 150, 100)
    h = blocks.res_free([1,1], t, TT=[30,60,90])
    T = 60
    assert (trapezoid(h,t)-T)**2/T**2 < 1e-9
...
def test_conc_free():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_free(H,t,TT), J, t)
    C = blocks.conc(J, H, t=t, model='free', TT=TT)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    C = blocks.conc_free(J, H, t, dt=t[1], TT=TT)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C0 = convolution.conv(blocks.res_free(H,t,TT=TT), J, t)
    C = blocks.conc_free(J, H, t, TT=TT)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-12
...
def test_flux_free():
    ...
    t = np.linspace(0, 150, 20)
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_free(H,t,TT), J, t)
    Jo = blocks.flux(J, H, t=t, model='free', TT=TT)
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    Jo = blocks.flux(J, H, dt=t[1], model='free', TT=TT)
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    J0 = convolution.conv(blocks.prop_free(H,t,TT=TT), J, t)
    Jo = blocks.flux(J, H, t=t, model='free', TT=TT)
    assert np.linalg.norm(Jo-J0)/np.linalg.norm(J0) < 1e-12
...
def test_conc_ncomp_prop():
    ...
    T = [6,12]
    E = [[1,0],[0,1]]
    C0 = blocks.conc_comp(J[0,:], T[0], t)
    C1 = blocks.conc_comp(J[1,:], T[1], t)
    prec = [1e-1, 1e-2, 1e-3]
...
def test_conc_ncomp():
    ...
    prec = [1e-1, 1e-2, 1e-3]
    for i, dt_prop in enumerate([None, 0.1, 0.01]):
        C = blocks.conc(J, T, E, t=t, model='ncomp', dt_prop=dt_prop)
        assert np.linalg.norm(C[0,:]-C0)/np.linalg.norm(C0) < prec[i]
        assert np.linalg.norm(C[1,:]-C1)/np.linalg.norm(C1) < prec[i]

    # Compare both solvers for a few coupled systems
    E = [[1,0.5],[0.5,1]]
    C = blocks.conc_ncomp(J, T, E, t, solver='prop', dt_prop=0.01)
    C0 = blocks.conc_ncomp(J, T, E, t, solver='diag')
    ...
    T3 = [6,12,6]
    J3 = np.ones((3,len(t)))
    C = blocks.conc_ncomp(J3, T3, E, t, solver='prop', dt_prop=0.01)
    C0 = blocks.conc_ncomp(J3, T3, E, t, solver='diag')
    ...
    E = 0.1
    E = [[1-E,1],[E,0]]
    C = blocks.conc(J, T, E, t=t, model='ncomp', solver='prop', dt_prop=0.01)
    C0 = blocks.conc(J, T, E, t=t, model='ncomp', solver='diag')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C) < 1e-3
...
def test_flux_ncomp():
    ...
    T = [6,12]
    E = [[1,0],[0,1]]
    J0 = blocks.conc_comp(J[0,:], T[0], t) / T[0]
    J1 = blocks.conc_comp(J[1,:], T[1], t) / T[1]
    J = blocks.flux(J, T, E, t=t, dt_prop=0.01, model='ncomp')
...
def test_res_ncomp():
    ...
    T = [6,12]
    E = [[1,0],[0,1]]
    R0 = blocks.res_comp(T[0], t)
    R1 = blocks.res_comp(T[1], t)
    R = blocks.res_ncomp(T, E, t)
...
def test_prop_ncomp():
    ...
    T = [6,12]
    E = [[1,0],[0,1]]
    H0 = blocks.prop_comp(T[0], t) 
    H1 = blocks.prop_comp(T[1], t)
    H = blocks.prop_ncomp(T, E, t)
    ...
    J[1,:] = 2
    E = [[1,0.5],[0.5,1]]
    Jo = blocks.flux_ncomp(J, T, E, t, solver='prop', dt_prop=0.01)
    H = blocks.prop_ncomp(T, E, t)
...
def test_conc_2comp():
    ...
    T = [6,12]
    E = [[1,0.5],[0.5,1]]
    C0 = blocks.conc(J, T, E, t=t, model='ncomp') 
    C = blocks.conc_ncomp(J, T, E, t, dt_prop=0.1)
    ...
    # Test exceptions
    try:
        blocks.conc_ncomp(J, [6,0], E, t)
    except:
        assert True
    else:
        assert False
    C = blocks.conc_ncomp([[1,1],[1,1]], [1,1], [[1,0],[0,1]])
    assert C[0,0]==0
...
def test_flux_2comp():
    ...
    J0 = blocks.conc_comp(J[0,:], T[0], t) / T[0]
    J1 = blocks.conc_comp(J[1,:], T[1], t) / T[1]
    J = blocks.flux(J, T, E, t=t, model='ncomp')
    assert np.linalg.norm(J[0,0,:]-J0)/np.linalg.norm(J0) < 1e-3
    assert np.linalg.norm(J[1,1,:]-J1)/np.linalg.norm(J1) < 1e-3
    assert np.linalg.norm(J[1,0,:]) == 0
    assert np.linalg.norm(J[0,1,:]) == 0
...
def test_conc_nscomp():
    ...
    # Test exceptions
    try:
        blocks.conc_nscomp(J, T, t)
    except:
        assert True
    try:
        blocks.conc_nscomp(J, [T,T], t)
    except:
        assert True
    try:
        blocks.conc_nscomp(J, -T*np.ones(len(t)), t)
    except:
        assert True

    # Test functionality
    C = blocks.conc_comp(J, T, t)
    C0 = blocks.conc(J, T*np.ones(len(t)), t=t, model='nscomp')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 0.1
    
    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C = blocks.conc_comp(J, T, t)
    C0 = blocks.conc_nscomp(J, T*np.ones(len(t)), t)
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 0.1
...
def test_flux_nscomp():
    ...
    # Test functionality
    C = blocks.flux(J, T, t=t, model='comp')
    C0 = blocks.flux(J, T*np.ones(len(t)), t=t, model='nscomp')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 0.1
...
def test_conc_mmcomp():
    ...
    C = blocks.conc(J, Vmax, Km, t=t, model='mmcomp', solver='SM')
    C0 = blocks.conc_mmcomp(J, Vmax, Km, t, solver='prop')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2

    t = [0,5,15,30,60,90,150]
    J = np.ones(len(t))
    C = blocks.conc_mmcomp(J, Vmax, Km, t, solver='SM')
    C0 = blocks.conc_mmcomp(J, Vmax, Km, t, solver='prop')
...
def test_flux_mmcomp():
    ...
    Km = np.amax(C)/2
    Vmax = np.amax(C)/np.amax(T)
    C = blocks.flux(J, Vmax, Km, t=t, model='mmcomp', solver='SM')
    C0 = blocks.flux(J, Vmax, Km, t=t, model='mmcomp', solver='prop')
    assert np.linalg.norm(C-C0)/np.linalg.norm(C0) < 1e-2
...
def test_conc_2cxm():
    ...
    T = [6,12]
    E = 0.1
    C0 = blocks.conc(J, T, E, t=t, model='2cxm')
    E = [[1-E,1],[E,0]]
    C = blocks.conc(J2c, T, E, t=t, model='ncomp')
    ...
    C0 = blocks.conc(J, T, E, t=t, model='2cxm')
    T[0] = 0.1
    C1 = blocks.conc(J, T, E, t=t, model='2cxm')
    T[0] = 0.01
    C2 = blocks.conc(J, T, E, t=t, model='2cxm')
    T[0] = 0.0
    C = blocks.conc(J, T, E, t=t, model='2cxm')
    ...
    E = 0.99
    T[0] = 0.01
    C3 = blocks.conc(J, T, E, t=t, model='2cxm')
    E = 1
    T[0] = 0
    C = blocks.conc(J, T, E, t=t, model='2cxm')
    # Check convergence to T=0 solution
...
def test_flux_2cxm():
    ...
    T = [6,12]
    E = 0.1
    J0 = blocks.flux(J, T, E, t=t, model='2cxm')
    E = [[1-E,1],[E,0]]
    J1 = blocks.flux(J2c, T, E, t=t, model='ncomp')[0,0,:]
    ...
    J0 = blocks.flux(J, T, E, t=t, model='2cxm')
    E = 0.95
    J1 = blocks.flux(J, T, E, t=t, model='2cxm')
    E = 0.999
    J2 = blocks.flux(J, T, E, t=t, model='2cxm')
    E = 1
    J3 = blocks.flux(J, T, E, t=t, model='2cxm')
...
def test_conc():
    ...
    E = 0.1
    try:
        blocks.conc(J, T, E, t=t, model='blabla') 
    except:
        assert True
    else:
        assert False  
...
def test_flux():
    ...
    E = 0.1
    try:
        J1 = blocks.flux(J, T, E, t=t, model='blabla') 
    except:
        assert True
    else:
        assert False   
